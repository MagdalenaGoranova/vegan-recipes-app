...
export default function MyRecipesCard ({myRecipe, deleteMyRecipe}) {
    ...
    useEffect(() => {
        ratingService.getRate(user.accessToken, myRecipe._id)
            .then(result => {
                console.log(result);
    ...
    }, [myRecipe._id, user.accessToken,]);

    useEffect(() => {
        commentsService.getCommentsCount(user.accessToken, myRecipe._id)
            .then(result => {
    ...
    <div id="recipes-card-wrapper">
        ...
        <div className="recipes-inner-card">
            ...
            <div id="recipes-bottom-details">
                ...
                <p><i className="fa-regular fa-clock"></i>{myRecipe.hours && myRecipe.hours !== 0 ? myRecipe.minutes && myRecipe.minutes !== 0 ? myRecipe.hours + ':' + myRecipe.minutes: myRecipe.hours + 'h': myRecipe.minutes + 'min'}</p>
                </div>
